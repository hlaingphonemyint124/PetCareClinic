...
function ServiceCard({ svc, status, currentIndex, totalCount }) {
  ...
  return (
    <motion.div
      ...
      style={{ rotateX, rotateY, transformPerspective: 1000, pointerEvents: isActive ? 'auto' : 'none', transformStyle: 'preserve-3d' }}
    ...
    >
      ...
      <AnimatePresence>
        {isActive && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ delay: 0.1 }} className="absolute top-6 left-6 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: '#C9A84C', boxShadow: '0 0 10px rgba(201,168,76,0.9)' }} />
            <span className="text-[10px] uppercase tracking-[0.25em]" style={{ color: 'rgba(232,228,217,0.65)', fontFamily: 'monospace' }}>Available Now</span>
          </motion.div>
        )}
      </AnimatePresence>
      <AnimatePresence>
        {isActive && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute top-6 right-6 flex flex-col items-end">
            <span className="font-display text-3xl font-bold leading-none" style={{ color: 'rgba(201,168,76,0.25)' }}>{String(currentIndex + 1).padStart(2, '0')}</span>
            <span className="text-[10px] tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.2)' }}>/ {String(totalCount).padStart(2, '0')}</span>
          </motion.div>
        )}
      </AnimatePresence>
      <AnimatePresence>
        {isActive && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }} transition={{ duration: 0.38 }} className="absolute inset-x-0 bottom-0 p-6">
    ...
    <div className="hidden lg:flex w-[38%] flex-shrink-0 relative overflow-hidden"
      ...
      <div className="relative w-full h-full flex items-center pl-14 pr-6">
        {services.map((svc, index) => {
          ...
          return (
            <motion.div key={svc.id} className="absolute left-14 right-6" style={{ height: CHIP_H }}
              animate={{ y: wrapped * CHIP_H, opacity: Math.max(0, 1 - absW * 0.22) }}
...
export function ServicesSection() {
  ...
  useEffect(() => {
    if (isPaused) { clearTimeout(timerRef.current); return }
    scheduleNext()
    return () => clearTimeout(timerRef.current)
  }, [step, isPaused, scheduleNext])
  ...
  useEffect(() => {
    const onKey = (e) => {
      ...
      if (e.key === 'ArrowDown' || e.key === 'ArrowRight') setStep(p => p + 1)
      if (e.key === 'ArrowUp'   || e.key === 'ArrowLeft')  setStep(p => p - 1)
    }
  ...
  return (
    <section className="py-20 md:py-28 px-4 md:px-6 bg-bg-2 relative overflow-hidden" ref={ref}>
      <div className="absolute inset-0 pointer-events-none" style={{ backgroundImage: 'radial-gradient(rgba(201,168,76,0.06) 1px, transparent 1px)', backgroundSize: '32px 32px' }} />
      <div className="absolute inset-0 pointer-events-none" style={{ background: 'radial-gradient(ellipse 60% 40% at 50% 60%, rgba(201,168,76,0.04), transparent)' }} />
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, delay: 0.2 }}
          ...
          <div className="flex-1 relative flex items-center justify-center overflow-hidden"
            ...
            <div className="relative w-full" style={{ maxWidth: 360, aspectRatio: '9/14' }}>
              {SERVICES.map((svc, index) => (
                <ServiceCard key={svc.id} svc={svc} status={getCardStatus(index)} currentIndex={currentIndex} totalCount={count} />
              ))}
            </div>
            <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 z-30">
              {SERVICES.map((_, i) => (
                <button key={i} type="button" onClick={() => handleChipClick(i)} aria-label={`Go to service ${i + 1}`}
                  className="rounded-full transition-all duration-300"
                  style={{ width: i === currentIndex ? 22 : 6, height: 6, background: i === currentIndex ? '#C9A84C' : 'rgba(255,255,255,0.15)', boxShadow: i === currentIndex ? '0 0 8px rgba(201,168,76,0.6)' : 'none' }} />
              ))}
            </div>
          </div>
        </motion.div>
        <motion.div initial={{ opacity: 0, y: 20 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.5 }}
          ...
          <Link to="/services" className="btn-ghost px-8 py-3 w-full sm:w-auto text-center justify-center">View All {DEMO_SERVICES.length} Services →</Link>
          <Link to="/booking" className="btn-primary px-8 py-3 w-full sm:w-auto text-center justify-center">Book Appointment</Link>
        </motion.div>
      </div>
    </section>
  )
