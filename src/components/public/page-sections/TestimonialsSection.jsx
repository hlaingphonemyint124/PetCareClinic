...
function TestimonialCard({ name, handle, pet, body, initials, color, textColor, emoji }) {
  return (
    <div className="rounded-xl p-3.5 flex-shrink-0 transition-all duration-200 cursor-default testimonial-card"
      style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)' }}>
      <div className="flex items-center gap-2.5 mb-2.5">
        <div className="w-[34px] h-[34px] rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0"
          style={{ background: color, color: textColor, border: `1.5px solid ${textColor}40` }}>
          {initials}
        </div>
        <div>
          ...
          <p className="text-[10px] mt-0.5" style={{ color: 'var(--text-muted)' }}>{handle} · {pet}</p>
        </div>
      </div>
      <div className="mb-2"><StarRating size="sm" /></div>
      <p className="text-[11.5px] leading-relaxed" style={{ color: 'var(--text-muted)' }}>{body}</p>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-3.5 flex-shrink-0 overflow-hidden" style={{ width: '190px' }}>
      <div className="flex flex-col gap-3.5"
        style={{ animation: `${animationName} ${duration} linear infinite`, willChange: 'transform', contain: 'layout style' }}
    ...
    <section className="py-28 px-6 bg-bg-2 relative overflow-hidden" ref={ref}>
      ...
      <div className="max-w-5xl mx-auto relative z-10 flex flex-col items-center gap-10">
        <motion.div className="text-center"
          ...
          <SectionHeader eyebrow="Loved by Yangon families" title="What Pet Owners" highlight="Say" />
          <motion.div className="flex items-center justify-center gap-3 mt-4"
            ...
            <StarRating size="md" />
            <span className="font-display text-lg font-bold" style={{ color: 'var(--text-primary)' }}>4.9</span>
            <span className="text-sm" style={{ color: 'var(--text-muted)' }}>from 380+ Google reviews</span>
          </motion.div>
        </motion.div>

        <motion.div className="w-full relative overflow-hidden rounded-2xl"
          style={{ height: '420px', border: '1px solid var(--border-subtle)' }}
          initial={{ opacity: 0, scale: 0.97 }} animate={inView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.2, duration: 0.7 }}>
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none"
            style={{ width: '500px', height: '250px', background: 'radial-gradient(ellipse, rgba(201,168,76,0.04) 0%, transparent 70%)' }} />
          <div className="absolute inset-0 flex items-center justify-center" style={{ perspective: '600px' }}>
            <div className="flex gap-3.5"
              style={{ transform: 'translateX(-60px) translateY(10px) translateZ(-80px) rotateX(18deg) rotateY(-8deg) rotateZ(18deg)', transformStyle: 'preserve-3d' }}>
          ...
          {/* Edge fades using CSS var */}
          <div className="pointer-events-none absolute z-10 testimonials-fade-top"  style={{ top:0, left:0, right:0, height:'28%', background:'linear-gradient(to bottom, var(--bg-2), transparent)' }} />
          <div className="pointer-events-none absolute z-10 testimonials-fade-bottom" style={{ bottom:0, left:0, right:0, height:'28%', background:'linear-gradient(to top, var(--bg-2), transparent)' }} />
          <div className="pointer-events-none absolute z-10 testimonials-fade-left"  style={{ top:0, bottom:0, left:0, width:'18%', background:'linear-gradient(to right, var(--bg-2), transparent)' }} />
          <div className="pointer-events-none absolute z-10 testimonials-fade-right" style={{ top:0, bottom:0, right:0, width:'18%', background:'linear-gradient(to left, var(--bg-2), transparent)' }} />
        </motion.div>
      </div>
    </section>
  )
