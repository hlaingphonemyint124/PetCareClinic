...
function BlogSlider({ posts }) {
  ...
  const onTouchEnd = (e) => {
    ...
    if (Math.abs(dx) > 44 && Math.abs(dx) > dy * 1.5) {
      dx > 0 ? setActive(p => p + 1) : setActive(p => p - 1)
    }
  ...
  // No global keyboard listener — would steal arrows from page scroll
  ...
  return (
    ...
    >
      {/* ── ALL IMAGES stacked — CSS opacity, no layout shift ── */}
      {posts.map((p, i) => (
        <div
          key={p.id}
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to top, rgba(5,10,20,0.97) 0%, rgba(5,10,20,0.35) 50%, transparent 100%)',
            }}
          />
          <div
            className="absolute inset-0 pointer-events-none"
            style={{
              background:
                'linear-gradient(to right, rgba(5,10,20,0.5) 0%, transparent 55%)',
            }}
          />
        </div>
      ))}
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(circle at 0% 0%, rgba(201,168,76,0.14), transparent 65%)',
        }}
      />
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 px-6 md:px-10 pb-6 md:pb-10">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Title */}
            <h3
              className="font-display text-white text-2xl md:text-4xl font-semibold leading-tight mb-2 max-w-2xl
                group-hover:text-[#e8c870] transition-colors duration-300"
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              {/* Counter + arrows — stop propagation so these don't trigger nav */}
              <div
              ...
              >
                <button
                  ...
                  onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = 'rgba(232,228,217,0.5)' }}
                  aria-label="Previous"
                >
                  <ChevronLeft size={15} />
                </button>
      ...
      <div
        className="absolute right-5 top-1/2 -translate-y-1/2 flex flex-col gap-2 z-30"
        onClick={e => e.stopPropagation()}
      >
        {posts.map((_, i) => (
          <button
            ...
            style={{
              ...
              boxShadow: i === activeIdx ? '0 0 8px rgba(201,168,76,0.7)' : 'none',
            }}
          />
        ))}
      </div>

      {/* Hover "click to read" overlay hint */}
      <div
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse 60% 45% at 50% 60%, rgba(201,168,76,0.04), transparent)',
        }}
      />
        ...
        <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
          ...
          >
            <div className="inline-flex items-center gap-2 mb-3">
              ...
              <span className="text-[#C9A84C] text-xs font-semibold tracking-[0.14em] uppercase">
                Pet health insights
              </span>
              <div className="w-1 h-1 rounded-full bg-[#C9A84C]" />
            </div>
            <h2 className="font-display text-[clamp(1.8rem,4vw,3rem)] text-white">
